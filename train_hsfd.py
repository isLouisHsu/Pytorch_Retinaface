...
from data import (WiderFaceDetection, detection_collate, 
    EcustHsfdDetection, detection_collate_hsfd,
    ...
    cfg_re34_hsfd_finetune, cfg_re34_hsfd_not_finetune)
...
def train():
    ...
    trainset = EcustHsfdDetection(training_dataset, used_channels, preproc=train_preproc(img_dim, rgb_mean), mode='train')
    validset = EcustHsfdDetection(training_dataset, used_channels, preproc=valid_preproc(img_dim, rgb_mean), mode='valid')
    # trainset = EcustHsfdDetection(training_dataset, transformers=train_transformers(img_dim), mode='train')
    # validset = EcustHsfdDetection(training_dataset, transformers=valid_transformers(img_dim), mode='valid')
